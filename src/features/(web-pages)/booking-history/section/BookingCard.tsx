...
export const BookingCard = ({ booking }: BookingCardProps) => {
  ...
  return (
    <div className="bg-white rounded-[20px] p-6 border border-gray-100 shadow-sm hover:shadow-md transition-shadow group">
      <div className="flex flex-col gap-6">
        ...
        {/* Itinerary Section */}
        <div className="relative pl-8 space-y-6 border-b border-[#E5E7EB] pb-4">
          {/* Connecting Line */}
          <div className="absolute left-[11px] top-2 bottom-2 w-[1.5px] border-l-2 border-dashed border-gray-100" />

          <div className="relative">
            {/* Dot */}
            <div className="absolute -left-8 top-1 w-6 h-6 rounded-full bg-white flex items-center justify-center border-2 border-white z-10 shadow-sm">
        ...
        <div className="flex flex-wrap items-center justify-between gap-6">
          ...
          <Link href={`/booking-history/${booking.id}`}>
            <Button className="bg-primary hover:bg-primary/90 text-white font-medium px-8 h-12 rounded-xl transition-all shadow-lg shadow-primary/10 flex items-center gap-2 group/btn">
              View Details
            </Button>
          </Link>
        </div>
...
const MetricItem = ({ label, value, isPrice = false }: { label: string; value: string; isPrice?: boolean }) => (
  <div className="space-y-1">
    <p className="text-[10px] font-normal text-gray-600  tracking-widest leading-none">
      {label}
    </p>
    <p className={cn(
